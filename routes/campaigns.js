...
const { getIntegration, getIntegrationHealth } = require('../db/db');
...
router.get('/', (req, res) => {
  try {
    ...
    const integration_health = { meta: getIntegrationHealth(brand_id, 'meta') };
    res.json({ ok: true, period, campaigns, integration_health });
  } catch (err) {
...
router.get('/instagram', (req, res) => {
  try {
    ...
    // META_ISOLATION: Only return cached data when the integration is active
    // and the cache is fresh (< 3 hours). An errored integration has stale
    // data from a previous sync — surfacing it as current is misleading.
    const META_STALE_MS = 3 * 60 * 60 * 1000;
    const integ = getIntegration(brand_id, 'meta');
    const metaOk = integ && integ.status !== 'disconnected' && integ.status !== 'error';
    let instagram = null;
    if (metaOk) {
      const ig = meta.getInstagram(brand_id);
      if (ig) {
        const fetchedMs = new Date(ig.fetched_at).getTime();
    ...
    res.json({ ok: true, instagram, integration_health });
  } catch (err) {
