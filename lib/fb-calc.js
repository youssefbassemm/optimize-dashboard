...
function seasonProgress(today, start, end) {
  const s = start || SEASON.START;
  const e = end   || SEASON.END;
  const totalDays  = daysBetweenInclusive(s, e);
  const rawElapsed = daysBetweenInclusive(s, today);
  const elapsed    = clamp(rawElapsed, 0, totalDays);
  const remaining  = totalDays - elapsed;
  const percent    = (elapsed / totalDays) * 100;
  return { totalDays, elapsed, remaining, percent, start: s, end: e };
}
